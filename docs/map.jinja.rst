...
We will see a quick setup to configure the ``mariadb`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``mariadb`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``mariadb`` formulas under ``/srv/salt/mariadb/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/mariadb/parameters/dns:domain/
    ln -s dns:domain /srv/salt/mariadb/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/mariadb/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/mariadb/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``mariadb`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``mariadb`` formulas under ``/srv/salt/mariadb/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/mariadb/parameters/roles
...
We create a configuration for the role ``mariadb/server`` in ``/srv/salt/mariadb/parameters/roles/mariadb/server.yaml``:
...
We create another configuration for the role ``mariadb/client`` in ``/srv/salt/mariadb/parameters/roles/mariadb/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles mariadb/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles mariadb/client
...
.. code-block:: console
    ...
    mv /srv/salt/mariadb/parameters/dns:domain/  '/srv/salt/mariadb/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-mariadb-as-json:
      test.nop:
        - name: {{ mariadb | json }}
...
Here is an example based on `template-formula/mariadb/config/file.sls`_:
...
This ``sls`` file expose a ``mariadb`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ mariadb['winner'] }}
...
.. _template-formula/mariadb/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/mariadb/config/file.sls
